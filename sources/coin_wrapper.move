...
module spike_amm::coin_wrapper {
    ...
    public entry fun claim_and_sweep_poel() acquires WrapperAccount {
        ...
        // Check rewards before attempting to withdraw to avoid ENO_WITHDRAWABLE_REWARDS abort
        let rewards_info = dfmm_framework::iAsset::get_user_rewards(wrapper_addr);
        let (_, withdrawable, _, _, _) = dfmm_framework::iAsset::deconstruct_user_rewards(&rewards_info);
        
        if (withdrawable == 0) {
            return // Silently exit if there are no rewards yet, saving gas and avoiding the abort.
        };
